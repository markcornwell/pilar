...
(add-tests-with-string-output "error handlers"			      
   ...
   [(fixnum->char #t) => "error:primitive: arg must be a fixnum\n"]
   [(char->fixnum 42) => "error:primitive: arg must be a character\n"]
   [(char=? #\A #t) => "error:primitive: arg must be a character\n"]
   [(char=? #f #\B) => "error:primitive: arg must be a character\n"]   
   [(char=? #t 14) => "error:primitive: arg must be a character\n"]
   [(fxlognot #t) => "error:primitive: arg must be a fixnum\n"]
   [(fxlogand #t #f) => "error:primitive: arg must be a fixnum\n"]
   [(fxlogor #t 12) => "error:primitive: arg must be a fixnum\n"]
   [(fxadd1 #f) => "error:primitive: arg must be a fixnum\n"]
   [(fxsub1 #\M) => "error:primitive: arg must be a fixnum\n"]
   [(fxzero? #f) => "error:primitive: arg must be a fixnum\n"]
   [(fx= #f 4) => "error:primitive: arg must be a fixnum\n"]
   [(fx= 4 #t) => "error:primitive: arg must be a fixnum\n"]
   [(fx<= #f 3) => "error:primitive: arg must be a fixnum\n"]
   [(fx<= 3 #t) => "error:primitive: arg must be a fixnum\n"]
   [(fx< #f 3) => "error:primitive: arg must be a fixnum\n"]
   [(fx< 3 #t) => "error:primitive: arg must be a fixnum\n"]
   [(fx>= #f 3) => "error:primitive: arg must be a fixnum\n"]
   [(fx>= 3 #t) => "error:primitive: arg must be a fixnum\n"]
