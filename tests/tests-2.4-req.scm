...
;; Commented out since there is no letrec* in R5RS.
;; Must be something from R6RS. --MRC
;;
;; (add-tests-with-string-output "letrec*"
;;   [(letrec* () 12) => "12\n"]
;;   [(letrec* ([f 12]) f) => "12\n"]
...
;;[(cond [#f] [#f 12] [12 13]) => "13\n"]
  [(cond [#f 42] [#f 12] [12 13]) => "13\n"]
  ;;[(cond [(cons 1 2) => (lambda (x) (cdr x))]) => "2\n"]
 ;; [(cond [(cons 1 2) (lambda (x) (cdr x))]) => "2\n"]  ;; <-- singlton clause I don't like
 [(cond [#t ((lambda (x) (cdr x))(cons 1 2))]) => "2\n"] 
  [(let ([else #t])
     (cond
      [else 1287])) => "1287\n"]
  ;; [(let ([else 17])
  ...
  ;; [(let ([else 17])
  ;;    (cond
  ;;     [else => (lambda (x) x)])) => "17\n"]   ;; <<- Decision not to support  B => E
  ;; [(let ([else 17])
  ;;    (cond
  ;;     [else (lambda (x) x)])) => "17\n"] 
  ;; [(let ([else #f])      ;; <<---- TRICKY rebinding else takes precedent?
  ;;    (cond 
